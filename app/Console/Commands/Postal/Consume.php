...
namespace App\Console\Commands\Postal;
...
class Consume extends Command
{
    ...
    protected $signature = 'postal:consume';
    ...
    public function __construct()
    {
        ...
        $this->s3Client = new S3Client([
            'region' => config('services.postal.region'),
            'credentials' => new Credentials(
                config('services.postal.key'),
                config('services.postal.secret'),
            ),
            'version' => 'latest'
        ]);
    ...
    private function handleMessage($queueMessage)
    {
        ...
        $object = $this->parseMessage($objectAsString);
    ...
    private function receiveMessage()
    {
        $result = $this->sqsClient->receiveMessage([
            ...
            'QueueUrl' => config('services.postal.queue_url'),
            'WaitTimeSeconds' => 20
        ]);
    ...
    private function getObjectAsString(string $key)
    {
        $result = $this->s3Client->getObject([
            'Bucket' => config('services.postal.bucket'),
            'Key' => $key,
        ]);
    ...
    private function parseMessage(string $messageAsString): array
    {
